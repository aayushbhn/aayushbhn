...
export const About = () => {
  ...
  return (
    <section id="about" className="relative w-full max-w-7xl mx-auto py-16 sm:py-24 md:py-32 px-4 sm:px-6 overflow-hidden">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 sm:gap-16 lg:gap-20 relative z-10">
        ...
        >
          <h2 className="text-3xl sm:text-4xl md:text-5xl font-extrabold mb-8 sm:mb-12 text-white">Career <span className="text-gradient">Trajectory</span></h2>
          <div className="space-y-10">
            {experiences.map((exp, index) => (
              <motion.div 
                key={index}
                whileHover={{ scale: 1.03, x: 10 }}
                className={`glass-card p-6 sm:p-8 rounded-3xl border-l-4 ${exp.highlight ? 'border-l-brand-400' : 'border-l-brand-800'}`}
              >
                <div className="flex flex-col md:flex-row md:items-center justify-between mb-4 gap-4">
                  <h3 className="text-lg sm:text-2xl font-bold text-white">{exp.title}</h3>
        ...
        >
          <h2 className="text-3xl sm:text-4xl md:text-5xl font-extrabold mb-8 sm:mb-12 text-white">Education <span className="text-gradient">Background</span></h2>
          <div className="space-y-8">
            {education.map((edu, index) => (
              <motion.div 
                key={index}
                whileHover={{ y: -5 }}
                className="bg-gradient-to-r from-brand-900/40 to-transparent p-6 sm:p-8 rounded-2xl border border-brand-800/30"
              >
